...
function loadConfigFile(
	...
	const envConfig =
		filename === CONFIG_FILENAME
			? process.env.RESEARCH_FABRIC_CONFIG
			: undefined;
...
function parseResearchFabricConfig(
	explicitPath?: string,
): ResearchFabricConfig {
	...
	if (!raw) {
		return {
			...
			preferDynamicFetchForDomains: [
				...DEFAULT_CONFIG.preferDynamicFetchForDomains,
			],
	...
	return {
		searchProviders: normalizeStringArray(raw.searchProviders) ?? [
			...DEFAULT_CONFIG.searchProviders,
		],
		fetchProviders: (
			normalizeStringArray(raw.fetchProviders) ?? [
				...DEFAULT_CONFIG.fetchProviders,
			]
		).map(normalizeFetchProvider),
		codeContextProviders: normalizeStringArray(raw.codeContextProviders) ?? [
			...DEFAULT_CONFIG.codeContextProviders,
		],
		siteMapProviders: normalizeStringArray(raw.siteMapProviders) ?? [
			...DEFAULT_CONFIG.siteMapProviders,
		],
		preferDynamicFetchForDomains: normalizeStringArray(
			raw.preferDynamicFetchForDomains,
		) ?? [...DEFAULT_CONFIG.preferDynamicFetchForDomains],
		cacheDir: resolvePath(
			getEnvValue("RESEARCH_FABRIC_CACHE_DIR") ??
				normalizeString(raw.cacheDir) ??
		...
		codeContextTtlMs:
			normalizeNumber(raw.codeContextTtlMs) ?? DEFAULT_CONFIG.codeContextTtlMs,
		siteMapTtlMs:
			normalizeNumber(raw.siteMapTtlMs) ?? DEFAULT_CONFIG.siteMapTtlMs,
	};
}
...
function parseExaConfig(): ExaConfig {
	...
	return {
		...
		tools: normalizeStringArray(getEnvValue("EXA_MCP_TOOLS")) ??
...
class ExaMcpClient {
		...
		if (!this.initializing) {
			this.initializing = (async () => {
				await this.sendRequest(
					...
					{
						protocolVersion:
							this.getConfig().protocolVersion ?? DEFAULT_PROTOCOL_VERSION,
						capabilities: {},
	...
	private async sendJsonRpc(
		...
		const { signal: mergedSignal, cleanup } = withTimeout(
			signal,
...
function extractJsonRpcResponse(
	...
	if (Array.isArray(response)) {
		const match = response.find(
			(item) =>
				isRecord(item) && item.id === requestId && item.jsonrpc === "2.0",
		);
...
function isProviderErrorResult(
	result: McpToolResult,
	renderedText: string,
): boolean {
	return (
		result.isError === true ||
		/^mcp error\b/i.test(renderedText.trim()) ||
		/^error\b/i.test(renderedText.trim())
	);
}
...
async function callBraveSearch(
	...
	try {
		...
		const json = await response.json();
		const results =
			isRecord(json) && isRecord(json.web) && Array.isArray(json.web.results)
				? json.web.results
				: [];
		...
		return results
			.map((result, index) => {
				...
				const age =
					normalizeString(result.age) ?? normalizeString(result.page_age) ?? "";
				return [
...
function prefersDynamicFetch(
	url: string,
	config: ResearchFabricConfig,
): boolean {
	try {
		const hostname = new URL(url).hostname.toLowerCase();
		const domains = config.preferDynamicFetchForDomains ?? [
			...DEFAULT_CONFIG.preferDynamicFetchForDomains,
		];
		return domains.some(
...
export default function researchFabric(pi: ExtensionAPI) {
	...
	const getResearchConfig = (): ResearchFabricConfig => {
		...
		return parseResearchFabricConfig(
	...
	const webSearchParams = Type.Object({
		...
		numResults: Type.Optional(
			Type.Number({ description: "Maximum number of results", default: 8 }),
		),
		freshness: Type.Optional(freshnessEnum),
		includeDomains: Type.Optional(
			Type.Array(Type.String(), { description: "Only include these domains" }),
		),
		...
		preferProvider: Type.Optional(
			providerEnum(
				["auto", "exa", "brave", "firecrawl"],
				"Preferred provider.",
			),
		),
		deep: Type.Optional(
			Type.Boolean({
				description: "Use deeper search if provider supports it",
			}),
		),
	});
	...
	const webFetchParams = Type.Object({
		...
		preferProvider: Type.Optional(
			providerEnum(
				["auto", "firecrawl", "direct", "brave"],
				"Preferred fetch provider.",
			),
		),
		renderJs: Type.Optional(
			Type.Boolean({
				description: "Render JavaScript when supported",
				default: true,
			}),
		),
		onlyMainContent: Type.Optional(
			Type.Boolean({
				description: "Extract only main content when supported",
				default: true,
			}),
		),
	});
	...
	const codeContextParams = Type.Object({
		...
		tokensNum: Type.Optional(
			Type.Number({
				description: "Approximate token budget for retrieved context",
			}),
		),
		preferProvider: Type.Optional(
			providerEnum(["auto", "exa"], "Preferred provider."),
		),
	});
	...
	const siteMapParams = Type.Object({
		...
		limit: Type.Optional(
			Type.Number({ description: "Maximum URLs to return" }),
		),
		search: Type.Optional(
			Type.String({ description: "Optional filter query for discovered URLs" }),
		),
		includeSubdomains: Type.Optional(
			Type.Boolean({ description: "Include subdomains", default: false }),
		),
		preferProvider: Type.Optional(
			providerEnum(["auto", "firecrawl"], "Preferred provider."),
		),
	});
	...
	const executeWebSearch = async (
		...
		const cached = readCache(
			cacheDir,
			key,
			config.searchTtlMs ?? DEFAULT_SEARCH_TTL_MS,
		);
		...
		const ordered =
			preferred !== "auto"
				? [
						preferred,
						...searchProviders.filter((provider) => provider !== preferred),
					]
				: searchProviders;
		...
		for (const provider of ordered) {
			try {
				if (provider === "exa") {
					...
					onUpdate?.({
						content: [{ type: "text", text: "WebSearch → Exa" }],
						details: { provider },
					});
				...
				if (provider === "brave") {
					...
					onUpdate?.({
						content: [{ type: "text", text: "WebSearch → Brave" }],
						details: { provider },
					});
				...
				if (provider === "firecrawl") {
					...
					onUpdate?.({
						content: [{ type: "text", text: "WebSearch → Firecrawl" }],
						details: { provider },
					});
				...
				onUpdate?.({
					content: [
						{
							type: "text",
							text: `WebSearch provider ${provider} failed: ${message}`,
						},
					],
	...
	const executeWebFetch = async (
		...
		const cached = readCache(
			cacheDir,
			key,
			config.fetchTtlMs ?? DEFAULT_FETCH_TTL_MS,
		);
		if (cached) return formatCached(cached);

		const preferred = normalizeFetchProvider(
			normalizeString(params.preferProvider) ?? "auto",
		);
		const url = String(params.url ?? "").trim();
		const dynamicHint = prefersDynamicFetch(url, config);
		const fetchProviders = (
			config.fetchProviders ?? [...DEFAULT_CONFIG.fetchProviders]
		).map(normalizeFetchProvider);
		const ordered =
			preferred !== "auto"
				? [
						preferred,
						...fetchProviders.filter((provider) => provider !== preferred),
					]
				: fetchProviders;
		...
		for (const provider of ordered) {
			try {
				if (provider === "firecrawl") {
					...
					if (!firecrawl.apiKey) continue;
					onUpdate?.({
						content: [{ type: "text", text: "WebFetch → Firecrawl" }],
						details: { provider },
					});
					const response = await callFirecrawl(
						firecrawl,
				...
				if (provider === "direct") {
					onUpdate?.({
						content: [
							{ type: "text", text: "WebFetch → direct fetch fallback" },
						],
						details: { provider },
					});
				...
				onUpdate?.({
					content: [
						{
							type: "text",
							text: `WebFetch provider ${provider} failed: ${message}`,
						},
					],
					details: { provider, failed: true },
				});
	...
	const executeCodeContextSearch = async (
		...
		const cached = readCache(
			cacheDir,
			key,
			config.codeContextTtlMs ?? DEFAULT_CODE_CONTEXT_TTL_MS,
		);
		if (cached) return formatCached(cached);

		const preferred = normalizeString(params.preferProvider) ?? "auto";
		const providers = config.codeContextProviders ?? [
			...DEFAULT_CONFIG.codeContextProviders,
		];
		const ordered =
			preferred !== "auto"
		...
		for (const provider of ordered) {
			try {
				if (provider === "exa") {
					...
					if (!exa.apiKey && !normalizeString(exa.url)) continue;
					onUpdate?.({
						content: [{ type: "text", text: "CodeContextSearch → Exa" }],
						details: { provider },
					});
					const result = await exaClient.callTool(
						"get_code_context_exa",
				...
				onUpdate?.({
					content: [
						{
							type: "text",
							text: `CodeContextSearch provider ${provider} failed: ${message}`,
						},
					],
					details: { provider, failed: true },
				});
	...
	const executeSiteMap = async (
		...
		const cached = readCache(
			cacheDir,
			key,
			config.siteMapTtlMs ?? DEFAULT_SITE_MAP_TTL_MS,
		);
		if (cached) return formatCached(cached);

		const preferred = normalizeString(params.preferProvider) ?? "auto";
		const providers = config.siteMapProviders ?? [
			...DEFAULT_CONFIG.siteMapProviders,
		];
		const ordered =
			preferred !== "auto"
		...
		for (const provider of ordered) {
			try {
				if (provider === "firecrawl") {
					...
					if (!firecrawl.apiKey) continue;
					onUpdate?.({
						content: [{ type: "text", text: "SiteMap → Firecrawl" }],
						details: { provider },
					});
					const response = await callFirecrawl(
						firecrawl,
				...
				onUpdate?.({
					content: [
						{
							type: "text",
							text: `SiteMap provider ${provider} failed: ${message}`,
						},
					],
					details: { provider, failed: true },
				});
